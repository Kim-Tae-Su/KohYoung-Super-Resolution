...
Calibration 이미지는 일반적인 RGB 이미지와 달리 이미지마다 픽셀 값의 분포 범위가 상이한 특성을 가진다
(예: [-50, 50], [-200, 200]).

이러한 데이터 특성으로 인해 전체 데이터셋 기준 Min-Max 정규화를 적용할 경우,
일부 이미지의 픽셀 값이 극단적으로 작거나 크게 왜곡되었으며,
그 결과 모델이 특정 이미지에 bias를 가지며 학습 성능이 저하되는 문제가 발생하였다.

이를 해결하기 위해 이미지 개별 기준 Min-Max Scaling 방식을 적용하였다.
각 이미지 단위로 정규화를 수행하여 모든 입력 이미지를 [0, 1] 범위로 정규화하였다.

정규화 수식:
X_norm = (X - X_min) / (X_max - X_min)

여기서 X_min, X_max는 개별 이미지 기준 최소값과 최대값이다.

또한 모델이 예측한 고해상도 이미지에 대해서는
각 이미지의 원래 Min / Max 값을 사용하여 역변환을 수행함으로써,
복원 후에도 물리적 해석이 가능한 Calibration 이미지를 유지하였다.
